...
def main() -> int:
    parser = argparse.ArgumentParser(description="HTTP benchmark compare: go/gin vs faio vs beast")
    ...
    go_dir = repo / "benchmark" / "http" / "gin-http"
    result_dir = repo / "benchmark"  / "result" / "http"

    targets = [
        {
            "name": "Go(Gin)",
            "build": lambda: (run_cmd(["go", "mod", "tidy"], go_dir), run_cmd(["go", "build", "-o", str(go_dir / "gin-benchmark"), "."], go_dir)),
            "start": lambda: subprocess.Popen([str(go_dir / "gin-benchmark"), "0.0.0.0", "9999"], cwd=str(go_dir), preexec_fn=os.setsid),
        ...
        {
            "name": "C++(faio)",
            "build": lambda: run_cmd(["cmake", "--build", str(build_dir), "-j4", "--target", "faio_http_benchmark"], repo),
            "start": lambda: subprocess.Popen([str(build_dir / "benchmark" / "faio_http_benchmark")], cwd=str(repo), preexec_fn=os.setsid),
            "health": "http://127.0.0.1:9998/health",
            "url": "http://127.0.0.1:9998/index",
            "port": 9998,
        },
        {
            "name": "C++(beast)",
            "build": lambda: run_cmd(["cmake", "--build", str(build_dir), "-j4", "--target", "beast_http_benchmark"], repo),
            "start": lambda: subprocess.Popen([str(build_dir / "benchmark" / "beast_http_benchmark"), "0.0.0.0", "9997"], cwd=str(repo), preexec_fn=os.setsid),
            "health": "http://127.0.0.1:9997/health",
            "url": "http://127.0.0.1:9997/index",
            "port": 9997,
        },
    ]
    ...
    try:
        for target in targets:
            ...
            stop_process(running, target["name"])
            running = None
        ...
        stop_process(running, "running-target")
